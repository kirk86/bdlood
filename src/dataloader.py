...
class Datasets(object):
    ...
    # ported from https://gist.github.com/ahmed-shariff/dbf3bee6fa2bf4924f95ac2b65620386
    ...
    def __init__(self,
        ...
        """
        Keyword arguments:
        train_dataset_file_path -- Path to the file with train dataset

        test_dataset_file_path -- Path to the file with test dataset.
        If this is None, a portion of the train dataset will be allocated
        as the test dataset based on the `test_size`.

        validation_dataset_file_path -- The path to the file
        containing the validation dataset.  If this is None, a portion
        of the train dataset will be allocated as the validation
        dataset based on the `validation_size` after allocating the
        test dataset.

        class_encoding -- Dict. The index to class name mapping of the
        dataset. Will be logged.

        train_data_load_function -- The function that will be used the
        content of the files passed above. This is a callable, that
        takes the file path and return the dataset.  The returned
        value should allow selecting rows using python's slicing (eg:
        pandas.DataFrame, python lists, numpy.array). Will be used to
        ...
        test_data_load_function -- Similar to
        `train_data_load_function`. This parameter can be used to
        define a seperate loading process for the test_dataset. If
        `test_dataset_file_path` is not None, this callable will be
        used to load the file's content. Also, if this parameter is
        set and `test_dataset_file_path` is None, instead of
        allocating a portion of the train_dataset as test_dataset, the
        files`train_dataset_file_path` passed will be loaded using
        this callable. Note that it is the callers responsibility to
        ensure there are no intersections between train and test
        dataset when data is loaded using this parameter.

        test_size -- Float between 0 and 1. The portion of the train
        dataset to allocate as the test dataset based if
        `test_dataset_file_path` not given and
        `test_data_load_function` is None.

        validation_size -- Float between 0 and 1. The portion of the
        train dataset to allocate as the validadtion dataset based if
...
class BaseTorchDataLoader(object):
