...
class KLDivergence(nn.Module):
    """Documentation for KLDivergence
    ...
    def forward(self, inputs, target):
        loss = torch.mean((inputs - target)**2)
        return loss
...
def main():
    ...
    x = Variable(torch.FloatTensor([[[1, 2, 3], [2, 3, 4], [3, 4, 5]], [
                 [1, 2, 3], [2, 3, 4], [3, 4, 5]]]).view(1, 2, 3, 3), requires_grad=True)
    addition = TVLoss()
    z = addition(x)
    print x
    print z.data
